...
def test_decrypt_data_base64_invalid_chars(monkeypatch):
    """
    Ensure ValueError is raised for invalid base64 characters or spaces when validate=True.
    """
    monkeypatch.setenv("ENCRYPTION_KEY", "any-key")
    invalid_b64 = "MTIzNDU2Nz g5MDEyhuA4"
    with pytest.raises(ValueError) as excinfo:
        decrypt_data(invalid_b64)
    assert "Invalid Base64 input" in str(excinfo.value)

def test_decrypt_data_invalid_utf8(monkeypatch):
    """
    Ensure ValueError is raised if decryption succeeds but content is not valid UTF-8.
    """
    passphrase = "my-secure-passphrase-2026"
    monkeypatch.setenv("ENCRYPTION_KEY", passphrase)
    key = derive_key(passphrase)
    iv = os.urandom(12)
    aesgcm = AESGCM(key)
    ciphertext_with_tag = aesgcm.encrypt(iv, b"\xff\xff\xff", None)
    combined = iv + ciphertext_with_tag
