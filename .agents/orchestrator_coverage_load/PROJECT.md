...
|---|------|-------|-------------|--------|
| 1 | M1: Code Coverage Configuration and Gaps Resolution | Add `pytest-cov`, configure pytest options in `pyproject.toml`, run tests, identify coverage gaps in `app/`, implement unit tests to bridge gaps to >90% coverage. | None | DONE |
| 2 | M2: Load Simulation Script Development | Implement `scripts/simulate_load.py` using `asyncio`/`httpx`, verify debounce/locks logic, ensure webhook response time < 500ms, and report results. | M1 | DONE |
